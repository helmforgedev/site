...
- **Blocklist presets** — `none`, `basic` (170k), `balanced` (970k), `aggressive` (2.6M), `gaming-friendly`; HaGeZi presets use Pi-hole v6-compatible AdBlock sources
...
| ------------------ | ------ | ------------------------- | ------------------ |
| `image.repository` | string | `docker.io/pihole/pihole` | Pi-hole image.     |
| `image.tag`        | string | `"2026.07.2"`             | Pinned image tag.  |
...
to `/etc/logrotate.d/pihole`. Back up `/etc/pihole` and `/etc/dnsmasq.d` before
