...
- Official `docker.io/clickhouse/clickhouse-server` image pinned to `26.6.1`
...
This chart now uses ClickHouse 26.6 stable instead of the 25.8 LTS line. Before
upgrading a production release, review the upstream 26.6 release notes and test
application queries, persisted data, and backup restoration. Take a verified
backup before allowing the StatefulSet rolling update to reuse the existing
data volume.
...
| `image.tag`                            | `26.6.1`                                 | Official full-version image tag.                           |
