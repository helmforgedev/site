...
<Callout type="info" title="External secret managers">
  This chart does not render ExternalSecret resources. Use `auth.existingSecret` with a Kubernetes Secret synchronized
  by your external secret manager when credentials should be owned outside Helm.
</Callout>
