...
This chart pins Jenkins `2.568.3-lts-jdk21`. Back up `JENKINS_HOME` before plugin, Java, or Jenkins LTS upgrades. Treat plugin upgrades as application changes:
review plugin changelogs, test JCasC reloads, and validate representative pipelines before promotion.

Jenkins 2.568.3 fixes core deserialization, request binding, CSRF, session, and
permission issues covered by the [September 2 security advisory](https://www.jenkins.io/security/advisory/2026-09-02/).
This image update retains JDK 21 and the existing plugin configuration.
...
| `image.tag`                        | `2.568.3-lts-jdk21`         | Pinned Jenkins LTS controller image tag.                                         |
