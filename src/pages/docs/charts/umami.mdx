...
`ghcr.io/umami-software/umami:3.3.1` image and supports a quick bundled PostgreSQL install as well as production
...
- **Umami 3.3.1** - aligned with the current HelmForge chart `appVersion` and validated with the HelmForge k3d runtime gate.
...
Umami waits for the configured PostgreSQL socket before starting. The readiness
container uses the pinned official BusyBox image by default, and its image and
pull policy can be replaced for private-registry or air-gapped environments:
...
Disable `initContainers.waitForDatabase.enabled` only when another platform
component guarantees database readiness, such as an injected init container or
an externally managed startup dependency.
...
| `image.tag`        | string | `3.3.1`                        | Umami image tag.                     |
...
| `initContainers.waitForDatabase.enabled`                 | boolean | `true`                                     | Wait for the database socket before Umami starts.                      |
| `initContainers.waitForDatabase.image`                   | string  | `docker.io/library/busybox:1.37`           | Image used by the database readiness check; private mirrors supported. |
| `initContainers.waitForDatabase.pullPolicy`              | string  | `IfNotPresent`                             | Pull policy for the database readiness image.                          |
...
Umami 3.3.1 normalizes usernames to lowercase during its database migration. Back up PostgreSQL before upgrading and
check for accounts whose usernames differ only by capitalization. This release also improves two-factor authentication
handling when `TWO_FACTOR_ENCRYPTION_KEY` is missing or invalid and fixes tracking session and root-path behavior.
