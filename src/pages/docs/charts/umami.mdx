...
`ghcr.io/umami-software/umami:3.3.0` image and supports a quick bundled PostgreSQL install as well as production
...
- **Umami 3.3.0** - aligned with the current HelmForge chart `appVersion` and validated with the HelmForge k3d runtime gate.
...
| `image.tag`        | string | `3.3.0`                        | Umami image tag.                     |
