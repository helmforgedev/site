...
<Callout type="danger" title="Valkey 9.1.1 security update">
  Valkey classifies 9.1.1 as an urgent security release. It fixes remote-code-execution risks in TLS connection handling
  through authenticated `CLIENT KILL` requests (CVE-2026-56684) and while loading a corrupt stream RDB (CVE-2026-63639).
  Upgrade all 9.1.0 deployments, especially those using TLS or importing untrusted RDB data.
</Callout>

## Key Features

- Official `docker.io/valkey/valkey` image pinned to `9.1.1`
...
| `image.tag`                                 | `9.1.1`                   | Pinned Valkey image tag.                                         |
...
| `tests.image.tag`                           | `9.1.1`                   | Pinned image tag for the Helm connection test.                   |
