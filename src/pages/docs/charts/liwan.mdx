...
Behind an Ingress or Gateway, set `LIWAN_TRUSTED_PROXIES` through
`liwan.extraEnv` to the actual proxy IPs or CIDRs. Multiple entries are
comma-separated, for example `127.0.0.1/8,10.42.0.0/16` only when those ranges
belong to your trusted proxies. Configure `LIWAN_TRUSTED_HEADERS` if your proxy
uses a different header. Avoid unrestricted `*` proxy trust.

Remove `LIWAN_USE_FORWARD_HEADERS`; upstream removed it and the chart now
rejects it with a migration message. Back up `/data` and review the
[official release](https://github.com/explodingcamera/liwan/releases/tag/liwan-v1.7.0)
before upgrading.

## Key Features

- **Official image** - pinned `ghcr.io/explodingcamera/liwan:1.7.0`
