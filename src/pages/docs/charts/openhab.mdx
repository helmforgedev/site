...
- **Namespace override** for GitOps workflows where Helm release metadata and runtime objects live in separate namespaces
...
Create the target namespace first, then set `namespaceOverride` when the Helm release namespace and openHAB workload namespace must be different:

```yaml
namespaceOverride: home-automation
```
...
| `karaf.service.ipFamilyPolicy` | Optional Karaf Service IP family policy    | `""`        |
| `karaf.service.ipFamilies`     | Optional Karaf ordered Service IP families | `[]`        |

### Gateway API

Use `gateway.enabled` when an existing Gateway controller owns ingress traffic and the chart should render an HTTPRoute for openHAB:
