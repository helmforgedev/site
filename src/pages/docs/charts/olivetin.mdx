...
| `image.tag`        | string | `"3000.19.0"`                  | Image tag.                           |
...
The chart pins OliveTin `3000.19.0`. Review the upstream changelog before
upgrading production workloads. The chart propagates `olivetin.port` through
`PORT`; remove duplicate `PORT` entries from `olivetin.extraEnv`.
Persistence-enabled installs create the data PVC when
