...
- **Authoritative application port** — `olivetin.port` configures Kubernetes and OliveTin through `PORT`
...
| `image.tag`        | string | `"3000.18.1"`                  | Image tag.                           |
...
| Parameter           | Type    | Default                 | Description                                                            |
| ------------------- | ------- | ----------------------- | ---------------------------------------------------------------------- |
| `olivetin.port`     | integer | `1337`                  | HTTP port propagated to OliveTin through the reserved `PORT` variable. |
| `olivetin.extraEnv` | array   | `[]`                    | Extra environment variables; do not add the chart-managed `PORT`.      |
...
OliveTin 3000.18 adds the `dnsname` argument type for RFC 1123 hostnames:
...
diagnostics, dashboard categories, persistent log filters, and `PORT` support.
It also fixes `shellAfterCompleted` output injection, HTTP timeout and path
traversal weaknesses, and unsafe theme permissions. Upstream reports no
breaking changes. The chart now propagates `olivetin.port` through `PORT`;
remove duplicate `PORT` entries from `olivetin.extraEnv`. Persistence-enabled
installs now create the data PVC when `persistence.existingClaim` is empty.
