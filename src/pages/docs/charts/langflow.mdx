...
The HelmForge Langflow chart uses the official `docker.io/langflowai/langflow:1.12.1` image and exposes the web/API server on port `7860`.
...
fixes component registry loading, flow-edge preservation, startup environment
synchronization, slow MCP connections and excessive file logging. Review provider
environment values before restarting existing deployments.
