...
The HelmForge Langflow chart uses the official `docker.io/langflowai/langflow:1.11.5` image and exposes the web/API server on port `7860`.
...
Langflow 1.11 introduces native v2 workflow execution, trusted JWT authentication with just-in-time user mapping, RBAC-aware interfaces,
A2A and human-in-the-loop workflows, and additional provider and vector-store bundles. Review the upstream changelog before upgrading between
1.11 patch releases. Version 1.11.5 adds security hardening and an opt-in multi-worker in-memory queue while keeping the
default runtime contract unchanged. Configure optional runtime capabilities through `app.env` or `app.envFrom`.
