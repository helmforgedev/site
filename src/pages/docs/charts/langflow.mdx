...
The HelmForge Langflow chart uses the official `docker.io/langflowai/langflow:1.11.4` image and exposes the web/API server on port `7860`.
...
1.11 patch releases. Version 1.11.4 includes dependency security remediation and fixes for MCP transactions, background runs, and chained
inputs. Configure optional runtime capabilities through `app.env` or `app.envFrom`.
