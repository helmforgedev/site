...
<Callout type="danger" title="Vaultwarden 1.37.3 compatibility update">
...
| `image.tag`        | string | `"1.37.3"`                     | Image tag.                           |
...
| `vaultwarden.proxy.trustedProxies`               | string  | `local`       | Peers allowed to supply the configured client IP header.                       |
| `vaultwarden.rateLimit.unauthenticated.seconds`  | integer | `60`          | Average seconds between protected unauthenticated requests from one IP.        |
| `vaultwarden.rateLimit.unauthenticated.maxBurst` | integer | `50`          | Shared burst budget for protected unauthenticated endpoints.                   |
...
| `networkPolicy.extraEgress`                | array   | `[]`    | Additional native egress rules when egress is enabled, for example SMTP or external databases. |
...
| `backup.s3.caSecret`                   | string  | `""`                                                               | Secret containing the private CA certificate.                       |
| `backup.s3.caKey`                      | string  | `ca.crt`                                                           | Key containing the CA certificate.                                  |
| `backup.s3.insecureSkipVerify`         | boolean | `false`                                                            | Disable S3 TLS verification; use only as an emergency escape hatch. |
