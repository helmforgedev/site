...
| `image.tag`        | string | `"2026.9.1"`                       | Image tag.                           |
...
| `tunnel.quickTunnel.protocol`   | string  | `"http2"`               | Quick tunnel edge transport: `http2`, `auto`, or `quic`.                                  |
...
| `probes.startup.failureThreshold`      | integer | `36`    | Startup probe failure threshold.    |
...
Quick tunnels use HTTP/2 by default to avoid requiring outbound UDP. Set
`tunnel.quickTunnel.protocol` to `auto` or `quic` when needed. Managed tunnels
retain the upstream transport default.

The startup probe allows 180 seconds of failed readiness checks plus the initial
delay, giving edge retries and protocol fallback time to complete. Readiness
still requires an active tunnel.

## Upgrading to 2026.9.1

Cloudflared 2026.9.1 fixes Access token locking and QUIC registration, edge
rotation and fallback handling. It updates Go and the distroless base image.
The deprecated `transport-loglevel` flag is restored after its removal in 2026.9.0. The chart retains its tunnel command and metrics endpoints; the startup probe
now allows more time for edge registration.
