...
<Callout type="warning" title="Complete the VectorChord migration before upgrading older installations">
  Immich 3 removes support for `pgvecto.rs`. If the installation predates Immich `1.133.0`, follow the upstream
  VectorChord migration first. Back up PostgreSQL and uploads before upgrading. The bundled PostgreSQL image already
  uses VectorChord, and the Immich server runs its application migrations during startup.
</Callout>
