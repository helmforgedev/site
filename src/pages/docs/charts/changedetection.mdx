...
description: changedetection.io Helm chart for Kubernetes website monitoring with persistent JSON datastore, notifications, External Secrets, Ingress, Gateway API, and optional Playwright rendering.
---
...
packages the official `ghcr.io/dgtlmoon/changedetection.io:0.60.5` image with persistent `/datastore` storage and an
optional Playwright browser sidecar for JavaScript-rendered pages.

## Architecture

The chart deploys a single `Deployment` with the `Recreate` strategy. The application stores its JSON settings and watch files,
...
| PVC                         | Stores JSON settings and watch data, snapshots, watch history, and Python user package installs. |
...
plus browser status-code, navigation, deadline and runaway-script fixes.
It restores locale generation for price formatting and reduces repeated
explicit garbage collection. Review [0.60.4](https://github.com/dgtlmoon/changedetection.io/releases/tag/0.60.4)
and [0.60.5](https://github.com/dgtlmoon/changedetection.io/releases/tag/0.60.5).
...
Keep `persistence.enabled=true` for any environment where watches or history matter. changedetection.io uses JSON settings and watch files,
...
| `image.tag`                                 | `0.60.5`                              | Pinned upstream image tag.                                              |
