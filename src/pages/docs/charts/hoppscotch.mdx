...
<Callout type="warning" title="WebSocket annotations required">
  Without the `proxy-read-timeout` and upgrade annotations, WebSocket connections will drop after 60 seconds,
  breaking real-time collaboration features.
</Callout>
...
<Callout type="warning" title="Never change after first install">
  Changing `DATA_ENCRYPTION_KEY` after data has been written will corrupt all encrypted records.
  Store it in a secret manager and use `encryption.existingSecret` in production.
</Callout>
