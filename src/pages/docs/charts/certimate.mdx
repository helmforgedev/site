...
The HelmForge Certimate chart uses the official `certimate/certimate:v0.4.31` image. Certimate listens on port `8090` and stores PocketBase state under `/app/pb_data`.
...
- `tmpStorage.sizeLimit`: optionally bounds `/tmp`; when set, the chart applies
  the same value to the container's ephemeral-storage request and limit.
...
Certimate 0.4.31 adds the Huawei iBMC provider and fixes Kubernetes Secret
deployment resource discovery. It does not change the container port, storage
path, or chart configuration contract. Review the
[upstream v0.4.31 release](https://github.com/certimate-go/certimate/releases/tag/v0.4.31)
before upgrading.
