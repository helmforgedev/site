...
<Callout type="warning" title="Gitea 1.27 upgrade">
  Gitea 1.27 changes reusable Actions workflow behavior and requires per-request CSP nonces for inline scripts. Review
  custom workflows, templates, themes, and embeds before upgrading. Back up repositories and the database first because
  Gitea applies required database migrations during the initial startup.
</Callout>
...
- [Chart design](https://github.com/helmforgedev/charts/tree/main/charts/gitea/DESIGN.md)
- [Architecture](https://github.com/helmforgedev/charts/tree/main/charts/gitea/docs/architecture.md)
- [Database modes and secrets](https://github.com/helmforgedev/charts/tree/main/charts/gitea/docs/database-modes.md)
- [Operations and backup](https://github.com/helmforgedev/charts/tree/main/charts/gitea/docs/operations-and-backup.md)
...
| `image.tag`        | string | `"1.27.3-rootless"`     | Rootless image tag (UID 1000). |
...
External Secrets integration reconciles admin credentials only. Set `admin.existingSecret` so the ExternalSecret owns
the target Secret and the chart does not create a competing admin Secret.
...
Gitea 1.27.3 contains security and maintenance fixes without a documented
change to the rootless image contract. Back up the database, repositories, and
LFS data before upgrading; Gitea applies required database migrations during
startup. Review the
