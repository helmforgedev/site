...
| `image.tag`        | string | `"2.34.4"`            | Image tag.           |
...
n8n `2.32.5` adds public API, workflow export, and editor capabilities, with
fixes for queue-mode MCP executions, S3 path signing, AI Assistant runs,
credentials, and integrations. Review the
...
`image.tag=2.34.4`; Helm otherwise retains the previous image override. Also
