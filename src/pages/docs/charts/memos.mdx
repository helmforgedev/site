...
- `replicaCount`: Memos pod count. Keep `1` with SQLite.
- `app.port`: HTTP port.
...
- The chart rejects SQLite scaling, external database without DSN, and external database without a data volume.
