...
- **Full Appwrite stack** — API, Console IV 1.1.78-self-hosted, Realtime WebSocket, 15 workers, schedulers, maintenance
...
<Callout type="info" title="Functions execution requires openruntimes-executor">
  The `workers.functions` and `schedulers.functions` workloads are deployed, but serverless function execution requires
  a separate `openruntimes-executor` service that is not included in this chart. The functions worker will start but
  function invocations will fail until the executor is available.
</Callout>
...
| `image.tag`                | string | `"2.1.0"`                     | Appwrite image tag.    |
| `console.image.repository` | string | `docker.io/appwrite/new`      | Console image.         |
| `console.image.tag`        | string | `"1.1.78-self-hosted"`        | Console image tag.     |
...
15 workers, each enabled by default at `replicaCount: 1`:
