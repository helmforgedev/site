...
The current chart defaults to ntfy 2.28.0. This release limits titles to 1 KiB,
combined tags to 512 bytes, and cache replay to 10 MiB. Cache replay counts
toward daily visitor bandwidth and may return HTTP 429; clients should handle
`X-Messages-Truncated` on capped replay responses. Existing values remain compatible. The
`--reset-then-reuse-values` flag retains an explicit `image.tag` from the current release; remove or update that override
if you want to adopt the chart's `v2.28.0` default:
...
| `image.tag`        | string | `"v2.28.0"`                    | Image tag.                           |
...
| `ntfy.authDefaultAccess`          | string  | `read-write`              | Default access for unauthenticated users: `read-write`, `read-only`, `write-only`, `deny-all`. |
