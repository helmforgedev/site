...
The current chart defaults to ntfy 2.27.0, with hardened message templates, verified-email login, and stable PostgreSQL
support. Existing values and the opt-in abuse ban-feed remain compatible. The
`--reset-then-reuse-values` flag retains an explicit `image.tag` from the current release; remove or update that override
if you want to adopt the chart's `v2.27.0` default:
...
| `image.tag`        | string | `"v2.27.0"`                    | Image tag.                           |
