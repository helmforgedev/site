...
title: Dreeve (Statistics for Strava) Chart
description: Dreeve Helm chart for Kubernetes fitness dashboards with SQLite storage, optional Strava imports, activity visualizations, persistence, Ingress, Gateway API, External Secrets, and dual-stack Services.
---

import DocsTabs from '../../../components/DocsTabs.astro';
import Callout from '../../../components/Callout.astro';

# Dreeve (Statistics for Strava)

Self-hosted fitness dashboard for [Dreeve](https://github.com/dreeveapp/dreeve), formerly Statistics for Strava.
...
| `image.tag`        | string | `"v5.0.0"`                                     | Image tag.                           |
...
## Upgrading to Dreeve 5

Dreeve 5 is a major upgrade that moves YAML settings into the database and
introduces a built-in admin panel. Before upgrading from v4, back up both
`storage/database` and the `config` directory, then follow the
[upstream migration guide](https://docs.dreeve.app/#/getting-started/migrating-from-v4).

- [Dreeve GitHub Repository](https://github.com/dreeveapp/dreeve)
