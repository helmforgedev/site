...
<Callout type="warning" title="Breaking feature release">
  The default image is now pinned to `docker.io/library/nginx:1.31.4`, image pull policy defaults to `IfNotPresent`, pod
...
<Callout type="info" title="NGINX 1.31.4 update">
  The default image adds stricter protocol validation, standards-correct HTTP/2 and gRPC upstream host handling, and
  worker stability and memory-safety fixes. No Generic chart configuration changes are required for this image update.
</Callout>
...
| `image.tag`            | `1.31.4`                  | Default image tag.                                            |
