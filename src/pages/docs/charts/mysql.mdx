...
- **Official MySQL image** - Uses `docker.io/library/mysql:26.7.0` by default.
...
| `image.tag`        | string | `"26.7.0"`                | MySQL runtime image tag.                                   |
...
| `backup.images.mysql.tag`                   | string  | `"26.7.0"`       | MySQL client image tag used for backup jobs.     |
...
MySQL 26.7.0 introduces Oracle's calendar-based version numbering after the 9.7
series and includes server, InnoDB, replication, Group Replication, and client
fixes. Back up all databases and run MySQL Shell's upgrade checker before
...
for removed or changed options, then validate authentication plugins,
replication state, and application queries in staging.

This remains an Innovation-to-Innovation upgrade, but direct downgrade from
26.7 to 9.7 is unsupported. Treat rollback as a restore operation: take and
verify a full pre-upgrade backup, then restore that backup into a 9.7 deployment
instead of reusing a data directory initialized or upgraded by 26.7.
