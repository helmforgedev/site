...
<Callout
  type="danger"
  title="Upgrading from Envoy Gateway chart 1.x to 2.0.0 requires a staged CRD migration"
>
  Do not upgrade directly with `crds.enabled=false`, uninstall the bundled
  CRDs, or replace the safe-upgrade policy.
...
the bundle or return the application to a pre-2.0.0 revision. Complete the
detailed **CRD Migration From Chart 1.10.1 to 2.0.0** procedure on this page
before changing production releases.
...
- **Gateway API Native**: First-class Gateway API support
  - Standalone Envoy Gateway v1.9.0 and Gateway API v1.6.1 Experimental CRD lifecycle
  - Fail-fast validation of all 18 required GVKs when `crds.enabled=false`
  - Working Gateway and HTTPRoute examples
  - Global `gatewayAPI.enabled` gate for helper resources
  - Automated GatewayClass creation
  - Support for the Gateway API v1.6.1 resources consumed by Envoy Gateway
...
- Namespace override for chart-managed namespaced resources
...
- Helm 3.17+ recommended for ownership migration
...
New installations use the [Envoy Gateway CRDs chart](https://helmforge.dev/docs/charts/envoy-gateway-crds/)
first, wait for discovery, then install this application with
`crds.enabled=false`. The local CRD dependency remains enabled by default only
as a migration bridge for existing chart 1.10.1 releases.

```bash
helm install envoy-gateway-crds helmforge/envoy-gateway-crds \
  --version 1.0.0 \
  --namespace envoy-gateway \
  --create-namespace
helm show crds helmforge/envoy-gateway-crds \
...
The built-in presets target the chart's Gateway. HTTPRoutes attached to that
Gateway inherit the policy; no `ExtensionRef` filter is required:
...
To let Prometheus discover the controller ServiceMonitor through a label selector,
...
The map defaults to `{}` and accepts string values. Built-in chart labels take
precedence when keys overlap. These labels apply only to ServiceMonitor metadata;
the selector used to find controller Services remains unchanged. Prometheus
`spec.serviceMonitorNamespaceSelector` must also allow the release namespace
(or the chart's `namespaceOverride`).
...
- **Egress**: Allow to Kubernetes API CIDRs on TCP 443/6443, DNS (53), and optional complete custom rules from `security.networkPolicy.extraEgress`
...
security:
  ...
  networkPolicy:
    # Narrow these defaults to the API endpoint CIDRs used by your cluster.
    apiServerCIDRs:
      - 10.43.0.1/32
    extraEgress:
      - to:
...
3. **Restrict route attachment** with the chart-managed Gateway:
   ...
   gateway:
     listeners:
       http:
         allowedRoutes:
           namespaces:
             from: Same # Only the Gateway namespace
   ```
...
| `crds.enabled`           | Migration bridge; disable after the standalone CRD release is established  | `true`   |
...
| `controller.image.tag`                 | Controller image tag          | `v1.9.1`                       |
...
| `proxy.image.tag`                        | Proxy image tag                                                 | `distroless-v1.39.0`           |
| `proxy.shutdownManager.image.repository` | Shutdown manager sidecar image                                  | `docker.io/envoyproxy/gateway` |
| `proxy.shutdownManager.image.tag`        | Shutdown manager sidecar image tag                              | `v1.9.1`                       |
...
| Parameter                       | Description                                       | Default                                       |
| ------------------------------- | ------------------------------------------------- | --------------------------------------------- |
| `gatewayAPI.enabled`            | Enable chart-managed Gateway API helper resources | `true`                                        |
| `gatewayAPI.examples.enabled`   | Create example resources                          | `true`                                        |
| `gatewayAPI.examples.namespace` | Namespace for examples                            | `""` (namespaceOverride or release namespace) |

### Gateway Resource

The managed Gateway does not allow any ListenerSets by default. To delegate
listener configuration to teams in the Gateway namespace, enable
...
The namespace policy also supports `None`, `All`, and `Selector`. When using
`Selector`, `gateway.allowedListeners.namespaces.selector` is required and must
contain a Kubernetes label selector. The chart bundles the supported Gateway
API v1.6.1 CRDs and requires the ListenerSet CRD, or an equivalent
platform-managed installation.

| Parameter                               | Description                                  | Default               |
| --------------------------------------- | -------------------------------------------- | --------------------- |
| `gateway.create`                        | Create default Gateway resource              | `true`                |
| `gateway.allowedListeners`              | ListenerSet namespace attachment policy      | `{}` (none)           |
| `gateway.listeners.http.enabled`        | Enable HTTP listener                         | `true`                |
| `gateway.listeners.http.allowedRoutes`  | Route kinds and namespaces allowed to attach | `{}` (same namespace) |
| `gateway.listeners.https.enabled`       | Enable HTTPS listener                        | `false`               |
| `gateway.listeners.https.allowedRoutes` | Route kinds and namespaces allowed to attach | `{}` (same namespace) |

### Certgen Job

| Parameter           | Description       | Default  |
| ------------------- | ----------------- | -------- |
| `certgen.image.tag` | Certgen image tag | `v1.9.1` |
...
| `rateLimiting.service.image.repository`      | Rate limit service image repository              | `docker.io/envoyproxy/ratelimit` |
| `rateLimiting.service.image.tag`             | Rate limit service image tag                     | `17b1956c`                       |
...
| `rateLimiting.externalRedis.auth.enabled`    | Read external Redis password from a Secret       | `false`                          |
| `rateLimiting.externalRedis.auth.secretName` | External Redis credential Secret                 | `""`                             |
| `rateLimiting.externalRedis.auth.secretKey`  | Password key in the credential Secret            | `password`                       |
...
| `security.networkPolicy.apiServerCIDRs` | CIDRs that may contain API endpoints; access is restricted to TCP ports 443 and 6443 | `["0.0.0.0/0","::/0"]`   |
| `security.networkPolicy.dns.namespace`  | Namespace containing cluster DNS pods allowed by NetworkPolicies                     | `kube-system`            |
| `security.networkPolicy.dns.podLabels`  | Labels selecting cluster DNS pods allowed by NetworkPolicies                         | `{"k8s-app":"kube-dns"}` |
...
| `serviceAccount.name`   | ServiceAccount name; must be `envoy-gateway` with rate limiting | `""`    |
...
`envoy-gateway` names for the controller Deployment and ServiceAccount. For an
externally managed ServiceAccount, set
...
with the Envoy Gateway v1.9.0 CRD bundle annotations. Apply the standalone CRDs
server-side before the controller upgrade and test existing Gateway, route,
EnvoyProxy, `BackendTrafficPolicy`, `SecurityPolicy`, EnvoyExtensionPolicy, and
EnvoyPatchPolicy resources in staging.

This release also corrects the declared Kubernetes support range to the
upstream v1.9 matrix: Kubernetes 1.33 through 1.36. Upgrade Kubernetes before
the chart when the cluster is outside that range.

### CRD Migration From Chart 1.10.1 to 2.0.0

Chart 1.10.1 installed 18 CRDs through Helm's special `crds/` directory and
owned the Gateway API safe-upgrade policy as a normal application template.
The CRDs themselves were never part of the Helm release manifest, but the
policy and binding were. Moving ownership therefore requires a bridge revision.

Chart 2.0.0 is intentionally a major release because it raises the Kubernetes
minimum from 1.26 to 1.33 and changes the CRD lifecycle and ownership contract.

Do not upgrade directly from 1.10.1 with `crds.enabled=false`. Do not install
the standalone policy over the policy owned by the application release.

#### 1. Capture the existing objects

Record all CRD and custom-resource names, UIDs, and counts before changing
ownership:
...
done | jq -s '{apiVersion: "v1", kind: "List", items: .}' > crds-before.json

for resource in $(kubectl api-resources \
  --api-group=gateway.networking.k8s.io --verbs=list -o name); do
  kubectl get "$resource" -A -o json
done > gateway-api-before.json

for resource in $(kubectl api-resources \
  --api-group=gateway.envoyproxy.io --verbs=list -o name); do
  kubectl get "$resource" -A -o json
done > envoy-api-before.json

kubectl get validatingadmissionpolicy,validatingadmissionpolicybinding \
  safe-upgrades.gateway.networking.k8s.io \
  -o json > policy-before.json

jq -r '.items[] | [.kind, .metadata.name, .metadata.uid] | @tsv' \
  policy-before.json | sort > policy-uids-before.tsv
```

Generate sorted name/UID inventories with `jq` and retain them with the change
record. The same loops must succeed after migration; a missing API type is a
preflight failure, not something to ignore.

#### 2. Upgrade to the bridge with the dependency enabled
...
Verify that both resources print `keep`:
...
This step must happen before disabling the dependency. Helm evaluates deletion
from the previous release manifest; patching only the live object is not enough.

#### 3. Apply the standalone bundle server-side
...
Do not add `--force-conflicts` without auditing the reported field managers and
paths.
...
Rendering now checks all 18 required GVKs. A server-connected operation also
checks Gateway API bundle `v1.6.1`/`experimental` and Envoy Gateway bundle
`v1.9.0` annotations. Do not bypass failures with `--disable-validation`.

#### 6. Transfer policy ownership

After verifying that the live and standalone policy specifications match, use
Helm 3.17 or later:
...
Older Helm clients can patch only
`app.kubernetes.io/managed-by=Helm`, `meta.helm.sh/release-name`, and
`meta.helm.sh/release-namespace` on the policy and binding after the same audit.
Never edit Helm release Secrets and never use a hook to transfer ownership.

#### 7. Verify preservation and runtime

Compare the post-migration inventory with the files captured in step 1. Capture
the policy objects again and require their UID diff to be empty:

```bash
set -euo pipefail
kubectl get validatingadmissionpolicy,validatingadmissionpolicybinding \
  safe-upgrades.gateway.networking.k8s.io \
  -o json > policy-after.json
jq -r '.items[] | [.kind, .metadata.name, .metadata.uid] | @tsv' \
  policy-after.json | sort > policy-uids-after.tsv
diff -u policy-uids-before.tsv policy-uids-after.tsv
```

All 18 CRD UIDs, all custom-resource UIDs/counts, and both policy-resource UIDs
must remain unchanged. Confirm that only the standalone release owns the
policy, the controller is Available, Gateways remain `Programmed`, and existing
traffic still succeeds.

After ownership transfer, do not roll back to an application revision from
before the bridge. Such a revision attempts to add the policy and binding back
to the old release. Deploy the required compatible application chart with
`crds.enabled=false` instead.

See the [standalone CRD lifecycle documentation](https://helmforge.dev/docs/charts/envoy-gateway-crds/)
for provider-managed Gateway API, uninstall behavior, Helmfile ordering, and
forward-only CRD upgrades.
...
```bash
# Install CRDs and the controller with the dev profile
helm install envoy-gateway helmforge/envoy-gateway \
  --set profile=dev
...
```bash
# Install Envoy Gateway and allow selected namespaces to attach Routes
helm install envoy-gateway helmforge/envoy-gateway \
  --set profile=production-ha \
  --set gatewayAPI.examples.enabled=false \
  --set gateway.listeners.http.allowedRoutes.namespaces.from=Selector \
  --set gateway.listeners.http.allowedRoutes.namespaces.selector.matchLabels.tenant=enabled
```
...
2. **Policy and route target different Gateways**:

   ```bash
   kubectl get backendtrafficpolicy <policy-name> -n <namespace> -o yaml
   kubectl get httproute <route-name> -n <namespace> -o yaml
   # The policy targetRef and route parentRef must identify the same Gateway
   ```
...
This breaking release adds the ownership-preserving migration to
`envoy-gateway-crds`, validates all required external APIs, and corrects
Kubernetes support to versions 1.33 through 1.36. Existing 1.10.1 users must
complete the bridge procedure above before setting `crds.enabled=false`.

### Version 1.10.1

Updates Envoy Gateway to v1.9.0, Envoy to distroless-v1.39.0, the rate-limit
service to `17b1956c`, and the supported Gateway API CRDs to v1.6.1. See the
upgrade notes for the CRD migration and changed upstream defaults.
...
- EG: v1.8.3 (was v1.0.0)
- Envoy: distroless-v1.38.0 (was v1.29.0)
- Redis: helmforge/redis subchart v1.6.19 (replaces inline StatefulSet)
- Gateway API CRDs: v1.5.1 (was v1.0.0)
