...
The managed Gateway denies external ListenerSets by default. To delegate
listener configuration to teams in the Gateway namespace, enable same-namespace
attachment:
...
The namespace policy also supports `None`, `All`, and `Selector`. ListenerSet is
an experimental Gateway API feature and requires the bundled experimental CRDs
or an equivalent platform-managed installation.
