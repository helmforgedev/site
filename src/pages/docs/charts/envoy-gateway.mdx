...
Compare the post-migration inventory with the files captured in step 1. All 18
CRD UIDs, all custom-resource UIDs/counts, and the policy UID must remain
unchanged. Confirm that only the standalone release owns the policy, the
controller is Available, Gateways remain `Programmed`, and existing traffic
still succeeds.
