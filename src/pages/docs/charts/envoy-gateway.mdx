...
description: Deploy Envoy Gateway on Kubernetes with HelmForge's production-ready Helm chart featuring Gateway API examples, cert-manager integration, and comprehensive observability.
---
...
- **Profile Presets**: Production-ready deployment configurations
  ...
  - Staging profile (2 replicas, self-signed TLS)
  - Production HA profile (DaemonSet, cert-manager, anti-affinity)

- **Gateway API Native**: First-class Gateway API support
  - Working Gateway and HTTPRoute examples
  - Automated GatewayClass creation
  - Support for all Gateway API v1 resources

- **Certificate Management**: Automated TLS with cert-manager
  - Profile-based cert-manager integration
  - Auto-provisioning for Gateway listeners
  - Self-signed and production issuer support
...
- **Comprehensive Observability**: Production-grade monitoring
  ...
  - PrometheusRule with 7 alert rules
...
- ✅ Support for Deployment and DaemonSet proxy modes
...
For pre-production testing with TLS:
...
```bash
kubectl wait --for=condition=Programmed gateway/envoy-gateway-example -n default --timeout=300s
```
...
| Profile           | Replicas      | Proxy Mode   | Resources    | TLS          | Use Case          |
| ----------------- | ------------- | ------------ | ------------ | ------------ | ----------------- |
| **dev**           | 1             | Deployment   | Minimal      | No           | Local development |
| **staging**       | 2             | Deployment   | Medium       | Self-signed  | Pre-production    |
| **production-ha** | 2 + DaemonSet | DaemonSet    | Production   | cert-manager | Production        |
...
controller:
  ...
  resources:
    requests:
      ...
      memory: 1Gi
proxy:
  mode: DaemonSet # via envoy-gateway.proxy.mode helper
highAvailability:
  enabled: true
certificates:
  certManager:
...
```bash
kubectl port-forward svc/envoy-gateway-proxy 8080:80
...
The chart automatically configures cert-manager based on the selected profile:

| Profile       | cert-manager Enabled | Default Issuer |
| ------------- | -------------------- | -------------- |
| dev           | No                   | -              |
| staging       | Yes                  | selfsigned     |
| production-ha | Yes                  | selfsigned     |
...
certificates:
  certManager:
    ...
    issuer: letsencrypt-prod # Issuer name
    issuerKind: ClusterIssuer # ClusterIssuer or Issuer
  autoProvision: false # Auto-create certificates for Gateway listeners
```
...
helm install envoy-gateway helmforge/envoy-gateway \
  --set profile=production-ha \
  --set certificates.certManager.issuer=letsencrypt-prod
```

#### Manual Certificate Creation

Create a certificate for your Gateway:
...
```bash
# View certificate
kubectl get certificate my-gateway-tls

# Check certificate details
kubectl describe certificate my-gateway-tls

# View certificate secret
kubectl get secret my-gateway-tls -o yaml
...
- **Pending state**: Check DNS configuration and ACME challenge
- **Invalid certificate**: Verify issuer configuration and email
- **Secret not found**: Ensure cert-manager created the secret successfully
...
1. **ServiceMonitors**: Scrape metrics from controller and proxy
2. **PrometheusRule**: 7 alert rules for common issues
...
The chart includes 7 PrometheusRule alerts:
...
| EnvoyGatewayProxyDown           | 2 min     | Critical | Proxy unavailable          |
...
- Gateway API resources (gateways, httproutes, etc.)
- Envoy Gateway CRDs (SecurityPolicy, BackendTrafficPolicy, etc.)
- Core resources (services, secrets, configmaps)
...
```bash
kubectl get clusterrole envoy-gateway
...
| ------------------ | ------------------------------------------------- | -------- |
| `profile`          | Profile preset (dev/staging/production-ha/custom) | `custom` |
...
| Parameter                         | Description                                | Default                      |
| --------------------------------- | ------------------------------------------ | ---------------------------- |
| `proxy.mode`                      | Deployment or DaemonSet                    | `Deployment`                 |
...
| `proxy.image.tag`                 | Proxy image tag                            | `v1.29.0`                    |
...
| `proxy.service.httpPort`          | HTTP port                                  | `80`                         |
| `proxy.service.httpsPort`         | HTTPS port                                 | `443`                        |
...
| Parameter                             | Description                 | Default         |
| ------------------------------------- | --------------------------- | --------------- |
| `certificates.certManager.enabled`    | Enable cert-manager         | `false`         |
| `certificates.certManager.issuer`     | Issuer name                 | `selfsigned`    |
| `certificates.certManager.issuerKind` | Issuer kind                 | `ClusterIssuer` |
| `certificates.autoProvision`          | Auto-provision certificates | `false`         |
...
| `rateLimiting.redis.image.tag`           | Redis image tag                   | `7.2-alpine`      |
...
### Scenario 2: Staging with Self-Signed TLS

Deploy for pre-production testing with TLS:

```bash
# Prerequisites: cert-manager installed
kubectl apply -f https://github.com/cert-manager/cert-manager/releases/download/v1.13.0/cert-manager.yaml

# Install with staging profile
helm install envoy-gateway helmforge/envoy-gateway \
  --set profile=staging

# Wait for certificate
kubectl wait --for=condition=Ready certificate/envoy-gateway-example-tls --timeout=300s
...
**Symptom**: Certificate not ready or HTTPS not working
...
3. **ACME challenge failed** (Let's Encrypt):
...
kubectl delete certificate envoy-gateway-example-tls
kubectl apply -f <certificate-manifest>

# Force renewal
kubectl delete secret envoy-gateway-example-tls
...
# Restart proxy pods
kubectl rollout restart deployment envoy-gateway-proxy
# Or for DaemonSet
kubectl rollout restart daemonset envoy-gateway-proxy
```
...
- cert-manager integration
- Comprehensive RBAC
- ServiceAccount and GatewayClass

**P2 (Production)**:

- Rate limiting with Redis backend
- Distributed rate limiting with presets (API: 100 req/min, Strict: 10 req/min)
...
If upgrading from a pre-release version, all new features are opt-in with no breaking changes.
