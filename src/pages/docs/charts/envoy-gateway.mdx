...
| `controller.image.tag`                 | Controller image tag          | `v1.9.0`                       |
...
| `proxy.shutdownManager.image.tag`        | Shutdown manager sidecar image tag                              | `v1.9.0`                       |
...
| Parameter           | Description       | Default  |
| ------------------- | ----------------- | -------- |
| `certgen.image.tag` | Certgen image tag | `v1.9.0` |
...
`v1.8.3`. The upstream image tag is published with the `v` prefix. This chart
...
Envoy Gateway v1.9.0 requires Gateway API v1.6.1 Experimental CRDs and adds stricter CEL
validation for client IP detection, API key extraction, and policy merge
targets. Lua extensions are disabled by default, tracing client sampling now
defaults to zero, and EndpointSlice indexing can increase controller memory use
on large clusters. Remove `sessionPersistence.idleTimeout` before applying the
new Gateway API CRDs, because Gateway API v1.6 removed that field. Review the
[upstream v1.9.0 release notes](https://gateway.envoyproxy.io/news/releases/notes/v1.9.0/)
before upgrading. Apply the standalone Gateway API v1.6.1 and Envoy Gateway
CRDs server-side before the controller upgrade, correct resources
rejected by the new validations, and test existing Gateway, route, EnvoyProxy,
`BackendTrafficPolicy`, `SecurityPolicy`, EnvoyExtensionPolicy, and
