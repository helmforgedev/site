...
- **Gateway API Native**: First-class Gateway API support
  - Envoy Gateway v1.9.0 and supported Gateway API v1.6.1 CRDs installed by default
  - `crds.enabled=false` support for platform-managed CRDs
...
- Kubernetes 1.26+ (Gateway API v1 support)
- Helm 3.8+
...
The chart installs Envoy Gateway v1.9.0 and its supported Gateway API v1.6.1
CRDs by default. This includes `ListenerSet`, `TCPRoute`, `TLSRoute`, and
`UDPRoute`, which are also present in the v1.6.1 Standard bundle. The bundled
definitions retain the upstream experimental-channel schema used by Envoy
Gateway v1.9.0. The separate `gateway.networking.x-k8s.io` experimental APIs
are excluded because their schema requires Kubernetes 1.32. Use
`--set crds.enabled=false` only when all required CRDs are managed externally.
...
| `crds.enabled`           | Install bundled Envoy Gateway and its supported Gateway API v1.6.1 CRDs    | `true`   |
...
Envoy Gateway v1.9.0 requires Gateway API v1.6 CRDs and adds stricter CEL
...
before upgrading. Apply the bundled supported Gateway API v1.6.1 and Envoy
Gateway CRDs server-side before the controller upgrade, correct resources
...
For platform-managed Gateway API CRDs, prefer `crds.enabled=false`; this
disables the entire CRD subchart, including its safe-upgrade admission policy.
Helm's `--skip-crds` flag skips files in the subchart's `crds/` directory but
still renders templates. When using `--skip-crds` and the platform also manages
the safe-upgrade policy, add
