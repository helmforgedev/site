...
```bash
helm install envoy-gateway oci://ghcr.io/helmforgedev/helm/envoy-gateway --version 1.10.0
```
...
CRDs by default. This includes the experimental `ListenerSet`, `TCPRoute`,
`TLSRoute`, and `UDPRoute` APIs. The unrelated `gateway.networking.x-k8s.io`
experimental APIs are excluded because their schema requires Kubernetes 1.32.
Use `--set crds.enabled=false` only when all required CRDs are managed
externally.
...
API v1.6.1 CRDs and requires the experimental ListenerSet CRD, or an equivalent
...
- EG: v1.9.0 (was v1.0.0)
- Envoy: distroless-v1.39.0 (was v1.29.0)
- Redis: helmforge/redis subchart v1.6.19 (replaces inline StatefulSet)
- Gateway API CRDs: v1.6.1 supported set (was v1.0.0)
