...
- **Gateway API Native**: First-class Gateway API support
  - Envoy Gateway and Gateway API experimental v1.5.1 CRDs installed by default
...
```bash
helm install envoy-gateway oci://ghcr.io/helmforgedev/helm/envoy-gateway --version 1.7.4
```
...
The chart installs the Envoy Gateway and Gateway API experimental v1.5.1 CRDs
by default. Use `--set crds.enabled=false` only when all required CRDs are
managed externally.
...
| `crds.enabled`           | Install bundled Envoy Gateway and Gateway API experimental CRDs            | `true`   |
...
| `controller.image.tag`                 | Controller image tag          | `v1.8.3`                       |
...
| `proxy.shutdownManager.image.tag`        | Shutdown manager sidecar image tag                              | `v1.8.3`                       |
...
contain a Kubernetes label selector. The chart bundles Gateway API v1.5.1
experimental-channel CRDs and requires the ListenerSet CRD, or an equivalent
...
| Parameter           | Description       | Default  |
| ------------------- | ----------------- | -------- |
| `certgen.image.tag` | Certgen image tag | `v1.8.3` |
...
| `rateLimiting.service.image.tag`             | Rate limit service image tag                     | `1e50889b`                       |
...
`v1.8.2`. The upstream image tag is published with the `v` prefix. This chart
