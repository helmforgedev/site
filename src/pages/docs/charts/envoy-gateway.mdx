...
### CRD Lifecycle Bridge (Current)

Adds the ownership-preserving migration to `envoy-gateway-crds`, validates all
required external APIs, and corrects Kubernetes support to versions 1.33 through
1.36. Existing 1.10.1 users must complete the bridge procedure above before
setting `crds.enabled=false`.
