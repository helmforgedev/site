...
| `image.tag`        | string | `"2.12.0"`                 | Image tag.  |
...
| `postgresql.startupProbe.initialDelaySeconds`   | integer | `30`           | Delay startup checks until PostgreSQL completes its temporary init server cycle.   |
| `postgresql.readinessProbe.initialDelaySeconds` | integer | `30`           | Delay readiness checks until PostgreSQL completes its temporary init server cycle. |
...
persistent storage, review the
[upstream 2.12.0 release](https://github.com/ckan/ckan/releases/tag/ckan-2.12.0),
and test extensions against CKAN 2.12 before production rollout. Keep Solr,
PostgreSQL, and Redis available while CKAN performs its upgrade steps.
