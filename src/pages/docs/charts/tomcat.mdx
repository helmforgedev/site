...
- Official Tomcat image pinned to `11.0.25-jdk17-temurin-noble`
...
Tomcat 11.0.25 fixes HTTP/2 prioritization, OpenSSL FFM symbol lookup, EL parser reuse, and a low-severity WebSocket
example denial of service. Custom clustering that uses `EncryptInterceptor` must account for its new AES-GCM default.
