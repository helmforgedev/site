...
- **Official Alpine image** — default `docker.io/library/rabbitmq:4.3.4-alpine` with chart-managed plugins
...
RabbitMQ 4.3.4 also hardens the Management UI Content Security Policy by removing `unsafe-eval` and `unsafe-inline`.
...
| `image.tag`        | string | `"4.3.4-alpine"`             | Official Alpine RabbitMQ image tag.  |
