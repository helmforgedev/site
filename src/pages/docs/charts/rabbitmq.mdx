...
- **Official Alpine image** — default `docker.io/library/rabbitmq:4.3.5-alpine` with chart-managed plugins
- **Hardened Management UI** — CSP excludes `unsafe-eval` and `unsafe-inline`, with a configurable Referrer-Policy
...
RabbitMQ 4.3.5 fixes quorum queue recovery, AMQP 1.0 validation, rolling-upgrade queries, direct reply-to duplicates,
and STOMP frame-size limits.
The chart renders `management.headers.referrer_policy = no-referrer` by default. Set
`management.referrerPolicy: ""` only when you intentionally want the browser's default policy.
...
| ------------------ | ------ | --------------- | --------------------------------------------------- |
| `architecture`     | string | `single-node`   | Deployment mode: one-node or clustered StatefulSet. |
...
| `image.tag`        | string | `"4.3.5-alpine"`             | Official Alpine RabbitMQ image tag.  |
...
| `management.referrerPolicy`           | string  | `no-referrer` | Referrer-Policy header; empty uses the browser default. |
