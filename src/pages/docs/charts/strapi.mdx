...
**Image:** `docker.io/helmforge/strapi-base:5.52.2`

### Key Features

- **Strapi 5.52.2** — Stable HelmForge base image with security patches
...
```dockerfile
# Dockerfile
FROM docker.io/helmforge/strapi-base:5.52.2
...
| `image.tag`                | `5.52.2`                | HelmForge Strapi base image tag                       |
...
Strapi `5.52.2` includes maintenance and security fixes plus an opt-in beta
media library. Existing deployments keep their current SVG upload behavior;
new upstream projects block SVG uploads by default. The HelmForge base image
preserves the existing runtime and database contract. Back up the database
before upgrading production workloads. If `strapi.upload.provider` is `local`,
also back up the uploads PVC.
