...
**Image:** `docker.io/helmforge/strapi-base:5.52.0`

### Key Features

- **Strapi 5.52.0** — Stable HelmForge base image with security patches
...
```dockerfile
# Dockerfile
FROM docker.io/helmforge/strapi-base:5.52.0
...
| `image.tag`                | `5.52.0`                | HelmForge Strapi base image tag                       |
...
Strapi `5.52.0` adds MCP action audit logging, Corsican locale codes, and Koa
proxy configuration. It fixes nested populate, relation editing, data-transfer
restore, and upload behavior, and updates `ws` to `8.21.2`, which includes the
fix for CVE-2026-48779. Back up the database and uploads PVC before upgrading
