...
- **Database connection pooling** — production-grade PostgreSQL management
- **Multiple database backends** — PostgreSQL by default, SQLite for development, or external databases
...
**Image:** `docker.io/helmforge/strapi-base:5.50.0`

### Key Features

- **Strapi 5.50.0** — Stable HelmForge base image with security patches
...
```dockerfile
# Dockerfile
FROM docker.io/helmforge/strapi-base:5.50.0
...
Default deployment for development or testing:
...
For single-pod local development with SQLite, set `database.mode: sqlite` and `postgresql.enabled: false`.
...
Bundled PostgreSQL for complete Helm release. With the default `database.mode: auto` and `postgresql.enabled: true`, the chart starts Strapi with PostgreSQL.
...
Bundled MySQL alternative for custom Strapi images that include the MySQL runtime driver:
...
| `image.tag`                | `5.50.0`                | HelmForge Strapi base image tag                       |
...
| Key                  | Default | Description                                                                            |
| -------------------- | ------- | -------------------------------------------------------------------------------------- |
| `database.mode`      | `auto`  | Database mode. `auto` resolves to PostgreSQL while the PostgreSQL subchart is enabled. |
...
Strapi `5.50.0` adds active device session management, secure defaults for new
projects created with `create-strapi-app`, EU SendGrid region support, and AWS
credential provider functions. Existing projects are not migrated to the new
scaffold defaults by changing the image tag. The release also defaults legacy
JWT verification to HS256 and includes admin, relation, upload, and dynamic-zone
fixes. Back up the database and uploads PVC before
