...
`docker.io/library/wordpress:7.1.0-apache` image. The chart keeps the default install useful for
...
The chart pins WordPress `7.1.0`. Review the
[official WordPress release archive](https://wordpress.org/news/category/releases/)
before upgrading production installations.

<Callout type="warning" title="Test plugins and themes before upgrading to WordPress 7.1">
  WordPress 7.1 is a major feature release with editor, media workflow, REST API, and developer-facing changes. Back up
  the database and `wp-content`, then verify plugin and theme compatibility in a non-production environment first.
</Callout>
...
| `image.tag`        | string  | `7.1.0-apache`                | Image tag.                               |
...
| `networkPolicy.egress.extraEgress`                | array   | `[]`                                                               | Complete custom egress rules.        |
