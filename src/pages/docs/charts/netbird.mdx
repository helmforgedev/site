...
For path-based routing, keep dashboard-only traffic on the dashboard service and send NetBird backend paths to the server service. The server paths include `/api`, `/oauth2`, `/relay`, `/signalexchange.SignalExchange/`, `/management.ManagementService/`, and `/management.ProxyService/`. Configure the ingress controller or gateway to preserve HTTP/2 or h2c behavior for gRPC routes.
