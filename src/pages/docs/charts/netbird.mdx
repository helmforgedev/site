...
self-hosting paths. It adds account settings for dashboard and agent-network
features, source and destination traffic filters, trusted-proxy handling for
relay client addresses, and a unified admin CLI. The redesigned desktop client
announced in this upstream release is distributed separately and is not part of
this server chart.

No chart value migration is required when upgrading from `0.74.7`. Back up the
...
subchart mode. Existing sqlite installs that want to keep sqlite should set:
