...
Production hardening should include stable existing Secrets, explicit CPU and
memory requests and limits, TLS ingress, and platform-level network policy
where required.
...
| `image.tag`        | string | `"v2.5.0"`                                  | Image tag.                           |
