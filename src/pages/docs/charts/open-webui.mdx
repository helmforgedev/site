...
<Callout type="warning" title="Open WebUI 0.9 introduced incompatible database migrations">
  Back up the database and persistent data before upgrading. Multi-replica and load-balanced installations must update
  all Open WebUI instances together; upstream does not support rolling updates while old and new database schemas are
  mixed.
</Callout>

Review the [Open WebUI 0.9 release notes](https://github.com/open-webui/open-webui/releases/tag/v0.9.0) before upgrading
...
| `image.tag`        | string | `"v0.11.0"`                     | Image tag.         |
