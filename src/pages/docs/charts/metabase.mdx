...
| `image.tag`        | string | `"v0.63.13"`                  | Image tag.                           |
...
Metabase v0.63.13 is the maintenance update selected by the upstream image
monitor. Back up the Metabase application database, keep the encryption key
stable, and review the
[official Metabase changelog](https://www.metabase.com/changelog/) before
upgrading. Validate `/api/health` after rollout; first startup can take several
minutes while database migrations run.
