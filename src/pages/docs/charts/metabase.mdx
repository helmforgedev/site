...
| `image.tag`        | string | `"v0.63.15"`                  | Image tag.                           |
...
Metabase v0.63.15 is a public maintenance release. Back up the Metabase
application database, keep the encryption key stable, and review the
[official Metabase 63 changelog](https://www.metabase.com/changelog/63#metabase-6315)
before upgrading. Validate `/api/health` after rollout; first startup can take
several minutes while database migrations run.
