...
The HelmForge Poznote chart uses the official `ghcr.io/timothepoznanski/poznote:6.68.1` image. The container serves the web frontend and API on port `80`, stores all data (notes, attachments, configuration) in a SQLite database under `/var/www/html/data`.
...
before upgrading. Set `poznote.sharing.hideRestrictUsers: true` if sharing
restriction controls should not be available in the UI.
