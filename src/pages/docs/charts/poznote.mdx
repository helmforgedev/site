...
The HelmForge Poznote chart uses the official `ghcr.io/timothepoznanski/poznote:6.45.0` image. The container serves the web frontend and API on port `80`, stores all data (notes, attachments, configuration) in a SQLite database under `/var/www/html/data`.
...
expanded user administration, per-user display controls, and editor and
sharing improvements introduced since `6.35.0`. Set
`poznote.sharing.hideRestrictUsers: true` if sharing restriction controls
should not be available in the UI.
