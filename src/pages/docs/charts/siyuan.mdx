...
title: SiYuan Chart
description:
  Private knowledge workspace with retained native authentication, strict OIDC admission, restricted runtime and
  verified complete-workspace recovery.
---
...
Open [http://localhost:6806](http://localhost:6806) and use the access code described in Helm NOTES. For remote access,
configure an HTTPS hostname and a WebSocket-capable ingress or Gateway. SiYuan is a personal administrative workspace;
this is not a tenant-isolated collaboration server.
...
See [values.yaml](https://github.com/helmforgedev/charts/blob/main/charts/siyuan/values.yaml) and
[values.schema.json](https://github.com/helmforgedev/charts/blob/main/charts/siyuan/values.schema.json) for the full
contract, including scheduling and dual-stack settings.
