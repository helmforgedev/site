...
- **Pinned upstream image** — official MongoDB `8.3.7` security and reliability patch
...
| `image.tag`        | string | `"8.3.7"`                 | Image tag.     |
...
When `auth.enabled: false`, the exporter now connects without root credentials using
`mongodb://localhost:27017/admin`. With authentication enabled, it keeps using the root
credentials from the chart-managed or existing secret.
...
MongoDB `8.3.7` is a security and reliability patch within the `8.3` release
line. It addresses upstream CVEs plus authorization, input validation,
...
before upgrading production clusters, take a backup, and verify existing PVC
data files are compatible with the target `mongod` version. Keep replica set
keyFiles and root credentials stable across `helm upgrade`; rotate them with
MongoDB administrative commands instead of changing chart values.
