...
- **Dedicated DNS service** — separate LoadBalancer for UDP/TCP port 53 with optional named ports such as DNS-over-TLS
...
| `image.tag`        | string | `"v0.107.78"`                   | Image tag.          |
...
| `service.dns.port`           | integer | `53`           | Built-in DNS service port, exposed over UDP and TCP.                                                |
| `service.dns.extraPorts`     | array   | `[]`           | Additional explicit `name`, `port`, `targetPort`, and `protocol` entries, such as DoT on TCP `853`. |
