...
| `image.tag`                                   | `9.5.3`                     | Elasticsearch version                                    |
...
| `kibana.image.tag`                            | `9.5.3`                     | Kibana version aligned with Elasticsearch                |
...
The chart pins Elasticsearch and Kibana to `9.5.3`. Review the
[upstream Elasticsearch release notes](https://www.elastic.co/docs/release-notes/elasticsearch)
before upgrading production clusters, take a snapshot backup, and verify
plugins, ILM policies, and index templates in a staging environment before
reusing existing PVCs.

Generated account credentials are retained on upgrades. A Kubernetes Secret
change alone does not rotate passwords already stored by Elasticsearch; use its
password change API and synchronize the Secret deliberately. TLS settings use
the mounted PEM key, certificate and CA files.
