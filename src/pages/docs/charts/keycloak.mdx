...
OpenID Connect, and SAML. The chart supports explicit `dev` and `production` modes, Keycloak `26.7.1`, bundled or
...
- **Keycloak 26.7.x alignment** — image tag and app version use `quay.io/keycloak/keycloak:26.7.1`.
...
| `image.tag`        | string | `"26.7.1"`                  | Keycloak image tag.                  |
