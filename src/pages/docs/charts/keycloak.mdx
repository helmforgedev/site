...
OpenID Connect, and SAML. The chart supports explicit `dev` and `production` modes, Keycloak `26.7.0`, bundled or
...
- **Keycloak 26.7.x alignment** — image tag and app version use `quay.io/keycloak/keycloak:26.7.0`.
...
| `image.tag`        | string | `"26.7.0"`                  | Keycloak image tag.                  |
