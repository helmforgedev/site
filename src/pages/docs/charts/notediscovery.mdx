...
The HelmForge NoteDiscovery chart uses the official `ghcr.io/gamosoft/notediscovery:0.31.5` image and deploys a single-writer web application.
...
- `notediscovery.defaultTheme`: theme shown to browsers without a saved preference.
- `notediscovery.pluginsDir`: writable plugin path; empty derives it from `persistence.mountPath`.
...
NoteDiscovery `0.31.5` improves nested-list presentation without changing the
deployment contract. See the
[upstream v0.31.5 release](https://github.com/gamosoft/NoteDiscovery/releases/tag/v0.31.5).

NoteDiscovery `0.31.4` adds folder and vault archive downloads and hardens
shared and exported pages against stored HTML injection. Logout is now
POST-only. Review the
[upstream v0.31.4 release](https://github.com/gamosoft/NoteDiscovery/releases/tag/v0.31.4)
before upgrading.

Search queries now require at least two characters. NoteDiscovery does not
process single-character searches in this release.

Generated configuration now keeps plugin state under the writable data volume
...
The chart sets `PLUGINS_DIR` to the effective `notediscovery.pluginsDir` for
generated config, existing Secrets, and External Secrets, while bootstrap
preserves files already present on the volume.
