...
The HelmForge NoteDiscovery chart uses the official `ghcr.io/gamosoft/notediscovery:0.28.4` image and deploys a single-writer web application.
...
operator-mounted theme ID. A theme already saved in the browser takes
precedence, and the application safely falls back to `light` for invalid IDs.
