...
- **Content persistence** — PVC-backed content directory for media, themes, routes, and redirects
...
<Callout type="info" title="Adapter paths">
  Ghost supports adapters installed outside the content directory. Keep custom adapters in the image rather than the
  content PVC so application code and user-managed content retain separate lifecycles.
</Callout>

<Callout type="info" title="Ghost 6.62.0 update">
  Ghost 6.62.0 adds the React tag details screen, updates bundled themes, and fixes member unsubscribe links and
  private-site landing pages without changing the official container contract. Back up content and the database, then
  validate themes, integrations, newsletters, comments, and member signup paths before upgrading production sites.
</Callout>
...
<DocsTabs
  tabs={[
    ...
    { id: 'custom-adapter', label: 'Custom Adapter Image' },
  ]}
>
...
```yaml
# values.yaml — external MySQL password synchronized by External Secrets Operator
ghost:
  url: 'https://blog.example.com'
...
<div data-tab-panel="custom-adapter">

```yaml
# values.yaml — custom Ghost image with adapters outside the content PVC
image:
...
Build `/opt/ghost/adapters` into the custom image. Keeping installed adapters
outside `/var/lib/ghost/content` prevents the content PVC from shadowing them
...
| `image.tag`        | string | `"6.62.0"`                | Image tag.                           |
...
<Callout type="tip" title="Keep installed adapters outside the content PVC">
  Ghost accepts `paths__installedAdaptersPath` for custom container images. Set it through `ghost.extraEnv` to the
  directory baked into the image. Do not place that directory under `/var/lib/ghost/content`, because the content PVC
  replaces that path at runtime.
</Callout>
...
Install [External Secrets Operator](https://external-secrets.io/latest/) separately before enabling this integration.
The chart renders `external-secrets.io/v1` resources and supports multiple complete definitions through
`externalSecrets.items`. Each item must set `name` or `fullnameOverride` so every resource has a unique identity.
...
| `externalSecrets.items`           | array   | `[]`    | Complete ExternalSecret definitions with metadata and `spec`.   |

<Callout type="warning" title="Keep the projected Secret name aligned">
  Set `database.external.existingSecret` to the same name rendered by the database ExternalSecret. Using
  `items[].fullnameOverride` provides a stable literal name. The matching item must project the configured password key
  through `spec.data`, or use `spec.dataFrom`. The legacy `secretStoreRef` and `data` fields remain compatible for
  existing releases, but new configurations should use `externalSecrets.items`.
</Callout>
...
| Parameter                   | Type    | Default             | Description                                                                   |
| --------------------------- | ------- | ------------------- | ----------------------------------------------------------------------------- |
| `persistence.enabled`       | boolean | `true`              | Enable a PVC for `/var/lib/ghost/content` (media, themes, routes, redirects). |
...
<Callout type="warning" title="S3 backup covers content files only — not the MySQL database">
  ...
  custom themes, `routes.yaml`, and `redirects.yaml`. It does **not** back up the MySQL database, which contains posts,
  members, settings, and newsletter data. Configure a separate MySQL backup strategy (e.g. the HelmForge MySQL chart
  backup feature) to protect the full Ghost dataset.
</Callout>
