...
<Callout type="info" title="Ghost 6.54.0">
  This release adds inline editing for routes and redirects, support for adapters installed outside the content
  directory, member labels, complimentary subscriptions, and member avatars in themes. It also hardens filename
  generation and fixes unsafe helper output and CSV import edge cases.
</Callout>
...
| `image.tag`        | string | `"6.54.0"`                | Image tag.                           |
...
<Callout type="tip" title="Keep installed adapters outside the content PVC">
  Ghost 6.54.0 accepts `paths__installedAdaptersPath` for custom container images. Set it through `ghost.extraEnv` to
  the directory baked into the image. Do not place that directory under `/var/lib/ghost/content`, because the content
  PVC replaces that path at runtime.
</Callout>
